...
func testServiceSourceEndpoints(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			"dual-stack load-balancer service gets both addresses",
			"",
			"",
			"testing",
			"foobar",
			...
			map[string]string{},
			map[string]string{
				hostnameAnnotationKey: "foobar.example.org.",
			},
			"1.1.1.2,2001:db8::2",
			[]v1.IPFamily{"IPv4", "IPv6"},
			[]string{},
		...
		{
			"IPv6-only load-balancer service gets IPv6 endpoint",
			"",
			"",
			"testing",
			"foobar-v6",
			...
			map[string]string{},
			map[string]string{
				hostnameAnnotationKey: "foobar-v6.example.org.",
			},
			"2001:db8::1",
			[]v1.IPFamily{"IPv6"},
			[]string{},
